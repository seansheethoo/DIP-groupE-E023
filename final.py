#This code was tested on Jupyter notebook
import cv2
import numpy as np
import glob
...
images_file = "challange_1" #change here to the challange folder number

net = cv2.dnn.readNet('yolov3_training_final.weights', 'yolov3_testing.cfg')
images_path = glob.glob(images_file+"\*.jpg")
...
images_path = [images_file+"//"+i+".jpg"for i in images_path] 
...
final_count.append(survivor_detected[0])

for x in range(1, n):
    imageA = cv2.imread(survivor_detected[x-1])
    print(survivor_detected[x-1])
    ...
    if(compare_images(imageA, imageB)):
        final_count.append(survivor_detected[x])
